...
export default  function LuxuryWatches ({...props}) {
    ...
    const {
        ...
        reFetch:refetchType
    } = useApi(()=> getTicketDetails(user.token,langVal,currency,'luxury_watches'), user !== null)
    useEffect(()=>{
        if (typeDetails) {
            setLoadData(false)
        }
    ...
    return (
        <div>
            <BuyTicketsComp
                ...
                type={'luxury_watches'}
                typeName={'Luxury Watches'}
                typeDetails={typeDetails}
                backgroundImgClass={'BG-5'}
            />
        </div>
    )
}
