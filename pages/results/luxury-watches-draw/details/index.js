...
export default  function LuxuryWatchesDetails ({...props}) {
    ...
    const {
        ...
        isLoading,
        reFetch
    } = useApi(()=>  getRaffellionaireDrawDetails(user.token,langVal,currency,draw_id,'luxury_watches'),router.isReady)

    useEffect(()=>{
        if (draw_id) reFetch(()=>  getRaffellionaireDrawDetails(user.token,langVal,currency,draw_id,'luxury_watches'))
    },[draw_id])
