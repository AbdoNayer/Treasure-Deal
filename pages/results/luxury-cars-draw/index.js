...
export default function LuxuryCarsDraw ({...props}) {
    ...
    const {
        ...
        error,
        reFetch
    } = useApi(()=> getResultsRaffellionaire(user.token,langVal,currency,'','luxury_cars'), user !== null)
    ...
    useEffect(()=>{
        if (dateValue) {
            reFetch(()=>getResultsRaffellionaire(user.token,langVal,currency,dateValue,'luxury_cars'))
        }
    },[dateValue])
