...
export default function LuxuryVillasDraw ({...props}) {
    ...
    const {
        ...
        error,
        reFetch
    } = useApi(()=> getResultsRaffellionaire(user.token,langVal,currency,'','luxury_villas'), user !== null)
    ...
    useEffect(()=>{
        if (dateValue) {
            reFetch(()=>getResultsRaffellionaire(user.token,langVal,currency,dateValue,'luxury_villas'))
        }
    },[dateValue])
