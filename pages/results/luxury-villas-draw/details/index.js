...
export default  function LuxuryVillasDetails ({...props}) {
    ...
    const {
        ...
        isLoading,
        reFetch
    } = useApi(()=>  getRaffellionaireDrawDetails(user.token,langVal,currency,draw_id,'luxury_villas'),router.isReady)

    useEffect(()=>{
        if (draw_id) reFetch(()=>  getRaffellionaireDrawDetails(user.token,langVal,currency,draw_id,'luxury_villas'))
    },[draw_id])
